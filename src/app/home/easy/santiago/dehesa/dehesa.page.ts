...
@Component({
  selector: 'app-dehesa',
  templateUrl: './dehesa.page.html',
  styleUrls: ['./dehesa.page.scss'],
})
export class DehesaPage implements OnInit {
  productos: Item = {
    nombre: '',
    precio: null,
    stock: null, 
  ...
  async save_easy_costanera(){
    ...
    const enlace = 'easy_dehesa';
    await this.database.createDocument<Item>(data, enlace, 'La Dehesa');
  ...
  getmostar(){
  this.database.getCollection('easy_dehesa').subscribe( res => {
