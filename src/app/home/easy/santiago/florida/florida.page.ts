...
@Component({
  ...
  templateUrl: './florida.page.html',
  styleUrls: ['./florida.page.scss'],
})
export class FloridaPage implements OnInit {
  productos: Item = {
    nombre: '',
    precio: null,
    stock: null, 
  ...
  async save_easy_costanera(){
    ...
    const enlace = 'easy_florida';
    await this.database.createDocument<Item>(data, enlace, 'La Florida');
  ...
  getmostar(){
  this.database.getCollection('easy_florida').subscribe( res => {
