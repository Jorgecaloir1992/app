...
@Component({
  selector: 'app-s-amalia',
  templateUrl: './s-amalia.page.html',
  styleUrls: ['./s-amalia.page.scss'],
})
export class SAmaliaPage implements OnInit {
  productos: Item = {
    nombre: '',
    precio: null,
    stock: null, 
  ...
  async save_easy_costanera(){
    ...
    const enlace = 'easy_S_Amalia';
    await this.database.createDocument<Item>(data, enlace, 'Santa Amalia');
  ...
  getmostar(){
  this.database.getCollection('easy_S_Amalia').subscribe( res => {
