...
@Component({
  selector: 'app-sodimac-macul',
  templateUrl: './sodimac-macul.page.html',
  styleUrls: ['./sodimac-macul.page.scss'],
})
export class SodimacMaculPage implements OnInit {
  productos: Item = {
  nombre: '',
  precio: null,
  stock: null, 
...
async save_easy_costanera(){
  ...
  const enlace = 'sodimac_macul';
  await this.database.createDocument<Item>(data, enlace, 'Macul');
...
getmostar(){
this.database.getCollection('sodimac_macul').subscribe( res => {
